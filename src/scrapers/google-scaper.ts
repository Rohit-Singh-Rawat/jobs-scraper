...
export class GoogleScraper extends BaseScraper {
	...
	public async scrape(): Promise<Job[]> {
		...
		try {
			...
			for (let pageNum = 1; pageNum <= scraperConfig.google.maxPages; pageNum++) {
				logger.info(`Scraping page ${pageNum}`);
				await page.goto(
					`https://www.google.com/about/careers/applications/jobs/results?sort_by=relevance&page=${pageNum}`,
					{
						waitUntil: 'networkidle2',
					}
				);
				await Promise.race([page.waitForSelector('li.lLd3Je'), page.waitForSelector('h2.r4nke')]);
				...
				for (const jobElement of jobElements) {
					try {
						const title = await jobElement.$eval('h3.QJPWVe', (el) => el.textContent?.trim() || '');
						...
						const location = await jobElement.$$eval('span.r0wTof', (elements) =>
						...
						jobs.push({
							title,
							description,
							uniqueIdentifier: await generateJobHash({ title, company: 'Google', location }),
							location,
							company: 'Google',
							jobUrl: url,
							createdAt: new Date(),
