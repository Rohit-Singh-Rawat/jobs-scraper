...
export class ScraperService {
  ...
  public async runAllScrapers(): Promise<void> {
    ...
    try {
      //  await Promise.all([
      // 		this.runMicrosoftScraper(),
      // 		this.runGoogleScraper(),
      // 		this.runAmazonScraper(),
      // 	]);

      // we can use the above code to run multiple scrapers in parallel but it will make puppeteer to make multiple browser process that can lead to performance issue

      // so we will run them sequentially at cost of execution time
