...
export class MicrosoftScraper extends BaseScraper {
	...
	private parsePostedDate(dateText: string): Date {
		try {
			const now = new Date();
			const match = dateText.match(/^(?:t)?(\d+)\s+(day|days|month|months|year|years)\s+ago$/);
	...
	public async scrape(): Promise<Job[]> {
		...
		try {
			...
			for (let pageNum = 1; pageNum <= scraperConfig.microsoft.maxPages; pageNum++) {
				...
				for (const jobElement of jobElements) {
					try {
						...
						const jobId = match ? parseInt(match[1]) : 0;
						const url = `https://careers.microsoft.com/us/en/job/${jobId}?l=en_us&pg=${pageNum}&pgSz=20&o=Relevance`;
						jobs.push({
							title,
							description,
							uniqueIdentifier: await generateJobHash({ title, company: 'Microsoft', location }),
							location,
